...
//varialble to keep track of where the blank location is
int xloc,yloc;
      ...
      for(int j=0; j<d; j++)
        {
            if(board[i][j]==tile)
            {
                ...
                if((board[xloc][yloc-1]==tile && yloc>-1) || (board[xloc+1][yloc]==tile && xloc<3)|| (board[xloc-1][yloc] ==tile && xloc>-1) || (board[xloc][yloc+1]==tile && yloc<3))
                {
                    board[a][b]=0;
